...
final class PatchCCOWithAppSwitchEligibility_Tests: XCTestCase {
    ...
    private let coreConfig = CoreConfig(clientID: "fake-client-id", merchantID: "test-merchant-id", environment: .sandbox)
    ...
    func test_patchCCO_constructsGraphQLRequest_variables_headers_and_clientContext() async throws {
        ...
        let result = try await sut.patchCCOWithAppSwitchEligibility(
            token: token,
            tokenType: tokenType,
    ...
    func test_patchCCO_bubblesNetworkingError() async {
        ...
        do {
            _ = try await sut.patchCCOWithAppSwitchEligibility(
                token: "t",
