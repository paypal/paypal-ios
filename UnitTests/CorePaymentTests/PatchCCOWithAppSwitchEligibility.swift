...
final class PatchCCOWithAppSwitchEligibility_Tests: XCTestCase {
    ...
    func test_patchCCO_bubblesNetworkingError() async {
        ...
        do {
            _ = try await sut.patchCCOWithAppSwitchEligibility(token: "t", tokenType: TokenType.clientToken.rawValue, paypalNativeAppInstalled: true)
    ...
    func test_patchCCO_success_returnsParsedEligibility_falseReasoned() async throws {
        ...
        let response = try await sut.patchCCOWithAppSwitchEligibility(token: "tok", tokenType: TokenType.clientToken.rawValue, paypalNativeAppInstalled: false)
